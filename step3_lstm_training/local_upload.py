...
def upload_to_hf_from_local(
    ...
    """
    Upload trained model, scaler, and config to Hugging Face Hub from local machine.
    ...
    try:
        ...
        if hf_token is None:
            return {
                "status": "error",
                "message": "HF token not found. Set HF_TOKEN env var or run 'huggingface-cli login'.",
            }
        ...
        print(
            f"[HF UPLOAD] Uploading to {repo_id}/{target_dir}/ from local machine..."
        )

        # Upload model
        print(f"[HF UPLOAD] Uploading {os.path.basename(model_path)}...")
        api.upload_file(
        ...
        # Upload scaler
        print(f"[HF UPLOAD] Uploading {os.path.basename(scaler_path)}...")
        api.upload_file(
        ...
        # Upload config
        print(f"[HF UPLOAD] Uploading {os.path.basename(config_path)}...")
        api.upload_file(
        ...
        print(
            f"[HF UPLOAD] All files uploaded successfully to {repo_id}/{target_dir}/"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: python local_upload.py <model_path> <scaler_path> <config_path> [symbol] [timeframe] [hf_token]"
        )
        print("  Example: python local_upload.py step3_output/model_15m.h5 step3_output/model_15m_scaler.pkl step3_output/model_15m_config.json BTCUSDT 15m")
        sys.exit(1)
    ...
    result = upload_to_hf_from_local(model_p, scaler_p, config_p, sym, tf, token)
