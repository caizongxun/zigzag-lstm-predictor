...
def _onehot_encode_classification(
    y_class: np.ndarray, num_classes: int = 4
) -> np.ndarray:
...
def _build_config_json(
    ...
    config = {
        ...
        "data_statistics": {
            ...
            "regression_mean_reference": float(
                np.log1p(391.5 if timeframe == "15m" else 99.6)
            ),
...
def train_test_model(timeframe: str = "15m", hf_token: str = None) -> Dict[str, Any]:
    """
    ...
    # Download features from HF
    print(f"[TEST TRAINER] Downloading features from HuggingFace...")
    try:
        features_dict = download_training_features(
            symbol="BTCUSDT", timeframes=[timeframe], hf_token=hf_token
        )
        features = features_dict[timeframe]
    except Exception as e:
        print(f"[TEST TRAINER] ERROR downloading from HF: {e}")
        raise

    X = features["X_sequences"]
    y_class = features["y_class"].astype(np.int32)
    y_reg = features["y_reg"].astype(np.float32)
    scaler = features["scaler"]

    print(f"[TEST TRAINER] Features downloaded successfully")

    # Load data
    print(f"[TEST TRAINER] Original dataset size: {len(X)} samples")

    # Use 10% of data for quick test
    fraction = TEST_CONFIG["data_fraction"]
    n_samples = int(len(X) * fraction)
    X = X[:n_samples]
    y_class = y_class[:n_samples]
    y_reg = y_reg[:n_samples]
    print(f"[TEST TRAINER] Using {n_samples} samples ({100*fraction:.1f}% of data)")
    ...
    model, history, results = compile_and_train(
        model, train_data, val_data, test_data, train_config
    )
    ...
    config_json = _build_config_json(
        timeframe, len(X), history, results, scaler, start_time
    )
...
if __name__ == "__main__":
    ...
    train_test_model(timeframe=timeframe, hf_token=hf_token)
