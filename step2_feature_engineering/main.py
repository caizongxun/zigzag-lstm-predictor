...
def process_single_file(csv_path: str, output_dir: Path, timeframe: str = "15m") -> dict:
    ...
    try:
        ...
        print(
            f"  Zigzag calculation complete: {zigzag_stats['total_points']} turning points found"
        )
...
def main():
    ...
    Note: Feature upload to HuggingFace is now a separate step.
    Run 'python step2_upload.py' to upload after processing.
    """
    print("\n" + "="*90)
    ...
    base_path = Path(
        r"C:\Users\zong\PycharmProjects\zigzag-lstm-predictor\step1_data_extraction\step1_output"
    )

    input_files = {
        "15m": base_path / "BTCUSDT_15m.csv",
        "1h": base_path / "BTCUSDT_1h.csv",
    }
    ...
    for timeframe, results in all_results.items():
        ...
        if results["success"]:
            ...
            print(
                f"    Valid sequences: {class_stats['valid']}/{stats['metadata']['total_sequences']} "
            ...
            print(
                f"    Valid sequences: {reg_stats['valid']}/{stats['metadata']['total_sequences']} "
    ...
    execution_log = {
        ...
        "results": all_results,
        "note": "Feature upload to HuggingFace is now a separate step. Run 'python step2_upload.py' after this script completes."
    }
    ...
    print(f"\n{'='*90}")
    print(
        f"Final Result: {success_count}/{total_count} files processed successfully"
    )
    ...
    print("\n" + "="*90)
    print("NEXT STEP: UPLOAD TO HUGGINGFACE")
    print("="*90)
    print("""
To upload features to HuggingFace dataset, run:
...
For detailed instructions, see: step2_upload_guide.md
""")
    print("="*90)
