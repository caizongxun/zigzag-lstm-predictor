...
def generate_classification_labels(df: pd.DataFrame, turning_points: List[Dict]) -> np.ndarray:
    ...
    Returns:
        np.ndarray: Classification labels array of shape (len(df),) with dtype int32
    """
    n = len(df)
...
def generate_regression_labels(df: pd.DataFrame, turning_points: List[Dict]) -> np.ndarray:
    ...
    Returns:
        np.ndarray: Distance to next turning point (in bars), or -1 if none, dtype int32
    """
    n = len(df)
    labels = np.full(n, -1, dtype=np.int32)
...
def create_sequences(
    features: np.ndarray, labels: np.ndarray, sequence_length: int = 30
    ...
    Returns:
        tuple: (X sequences, y labels)
               X: shape (n_sequences, sequence_length, n_features), dtype float32
               y: shape (n_sequences,), dtype same as labels
    """
    ...
    if len(features) < sequence_length:
        raise ValueError(
            f"Not enough data for sequence_length {sequence_length}. Data length: {len(features)}"
        )
...
def validate_sequences(
    ...
    if len(X.shape) != 3:
        print(
            f"Error: X should be 3D (samples, timesteps, features), got shape {X.shape}"
        )
        return False
    ...
    if np.isnan(X).any():
        ...
        print(
            f"Warning: {nan_count} NaN values found in X. This may cause training issues."
        )
...
def prepare_training_data(
    ...
    if len(features_df) != len(zigzag_df):
        raise ValueError(
            f"Features and zigzag DataFrames length mismatch: "
    ...
    if len(feature_columns) == 0:
        raise ValueError(
            f"No feature columns found in DataFrame. Available columns: {features_df.columns.tolist()}"
        )
    ...
    print(
        "  Generating classification labels (HH=0, LL=1, HL=2, LH=3, unknown=-1)..."
    )
    ...
    print(
        f"    Raw labels - Valid: {class_distribution_raw['valid']}, "
    ...
    print(
        f"    Raw labels - Valid: {reg_distribution_raw['valid']}, "
    ...
    print(
        f"    Classification - Valid: {valid_class}/{total_seq} "
    ...
    print(
        f"    Regression - Valid: {valid_reg}/{total_seq} "
        f"({reg_stats['valid_percent']}%)"
    )
    print(f"      Mean distance: {reg_stats['mean_distance']:.1f} bars")
    print(
        f"      Distance range: {reg_stats['min_distance']}-{reg_stats['max_distance']} bars"
    )
